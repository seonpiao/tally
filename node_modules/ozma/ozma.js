...
function ozma(opt) {
    ...
    oz.exec = function(list) {
        ...
        list.reverse().forEach(function(mod) {
            ...
            if (mod.url || !mid) {
                if (mod.built || !mid && !_is_global_scope) {
                    if (mod.built) {
        ...
        var output = _config.disableAutoSuffix ? _build_script : oz.namesuffix(_build_script);
        if (!_is_global_scope) {
        ...
        var _writeCallback = function(err) {
            ...
            if (!seek_lazy_module()) {
                logger.log(_output_count + ' files, built in ' + (+new Date() - _begin_time) + 'ms');
        ...
        if (!observers) {
            ...
            read(m, function(data) {
                ...
                if (data) {
                    ...
                    if (is_undefined_mod) {
                        if (_mods[m.name] === m) {
                            _code_cache[m.name] += '\n/* autogeneration */' + '\ndefine("' + m.name + '", [' + (m.deps && m.deps.length ? ('"' + m.deps.join('", "') + '"') : '') + '], function(){});\n';
                        } else {
    ...
    function seek_lazy_module() {
        if (!_lazy_loading.length) {
            ...
            if (clip[0]) {
                logger.log(STEPMARK, 'Analyzing dynamic dependencies inside', '"' + clip[0] + '"(included in', '\033[4m' + _current_scope_file + '\033[0m)');
    ...
    function auto_fix_anon(mod) {
        ...
        if (hiddenDeps.length) {
            _code_cache[mod.name] = _code_cache[mod.name].replace(RE_AUTOARGS, function($0, $1, $2) {
                return 'define(' + $1 + (hiddenDeps.length ? ('["' + hiddenDeps.join('", "') + '"]') : '[]') + ', function(' + ($2 && hiddenDeps.length ? hiddenDeps.map(function(n, i) {
                    return '__oz' + i;
                }).join(', ') + ', ' : '') + $2 + '){';
            });
        }
    ...
    function load_config(file) {
        ...
        try {
            ...
            logger.error("\033[31m", 'ERROR: Can not parse', file, ' [' + ex.toString().replace(/\s*\n/g, '') + ']', "\033[0m");
            throw ex;
        }
    ...
    function main(args, callback, opt) {
        ...
        if (!_config["baseUrl"]) {
            ...
            if (input_config) {
                cfg = typeof input_config === 'string' ? load_config(args['config']) : config(_config, input_config, _DEFAULT_CONFIG);
            }
            if (!cfg) {
            ...
            if (args['jam']) {
                ...
                fs.readFile(jam_path + 'require.config.js', 'utf-8', function(err, data) {
                    ...
                    var autoconfig = _runtime.jam.packages.map(function(m) {
                        return 'define("' + m.name + '", "' + path.join(jam_dir, (/[^\/]+$/.exec(m.location)[0]), m.main) + '");\n';
                    }).join('');
